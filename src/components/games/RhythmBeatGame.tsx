...
const RhythmBeatGame: React.FC = () => {
  ...
  const compareMessage = useMemo(() => {
    ...
    const { sessionAccuracy, previousBest, savedBest, isNewBest } = resultCompare;
    if (previousBest === null) return '첫 기록이 저장되었습니다!';
    if (isNewBest && sessionAccuracy > previousBest) {
  ...
  const saveScore = useCallback(
    async (accuracy: number, avgErrorMs: number, sessionBpm: number) => {
      ...
      const previousBest =
        buildLeaderboard(bestScores, platform).find((e) => e.uid === user.uid)?.bestAccuracy ?? null;
  ...
  const handleTap = useCallback(() => {
    if (phaseRef.current !== 'playing' || !isScoringRef.current) return;
    ...
    for (let i = 0; i < SCORED_BEAT_COUNT; i++) {
      ...
      if (now < beatTime - EARLY_TAP_WINDOW_MS) continue;
    ...
    if (bestIdx < 0 || bestError > SCORING_WINDOW_MS) return;

    const rounded = Math.round(bestError);
    tapResultsRef.current.set(bestIdx, rounded);
